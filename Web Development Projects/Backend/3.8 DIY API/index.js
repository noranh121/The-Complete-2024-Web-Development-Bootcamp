...
app.put("/jokes/:id", (req, res) => {
  ...
  const currJoke = jokes.findIndex((joke) => joke.id === id);
  jokes[currJoke] = newJoke;
  res.json(newJoke);
});
...
app.patch("/jokes/:id", (req, res) => {
  ...
  const updatedJoke = {
    id: id,
    text: req.body.jokeText || jokes[currJoke].text,
    type: req.body.jokeType || jokes[currJoke].type,
  };
...
app.delete("/jokes/:id", (req, res) => {
  ...
  const currJoke = jokes.findIndex((joke) => joke.id === id);
  if (currJoke > -1) {
    jokes.splice(currJoke, 1);
    res
    .sendStatus(200)
    .json({ message: `Joke with id ${id} deleted.` });
  } else {
    res
    .status(404)
